    ...
    <div class="filter-container">
      <el-form :inline="true" :model="search">
        ...
        <el-form-item label="提测人">
          <el-input v-model="search.developer" placeholder="默认rd" style="width: 210px;" clearable />
        </el-form-item>
    ...
    <div>
      <el-dialog :title="requestInfo.title" :visible.sync="requestInfoVisible">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="提测版本">{{ requestInfo.test_version }}</el-descriptions-item>
          <el-descriptions-item label="提测类型">{{ formatInfoType(requestInfo.type) }}</el-descriptions-item>
          <el-descriptions-item label="应用名称" :span="2">{{ requestInfo.appId }}</el-descriptions-item>
...
import { apiMyTestSearch, changeStatus } from '@/api/mytest.js'
...
export default {
  ...
  methods: {
    ...
    searchClick() {
      ...
      apiMyTestSearch(body).then(response => {
        // 将返回的结果赋值给表格自动匹配
        var data = response.data
        // var data_new = []
        // data = data.filter(function(item) { return item.tester === this.op_user })
        // for (var test in data) {
        //   if (data[test].tester === this.op_user) {
        //     data_new.push(data[test])
        //   }
        // }
        //
