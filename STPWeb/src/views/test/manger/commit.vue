<template>
  <div class="app-container">
    <el-header>
      <el-page-header @back="goBack" :content="Action==='UPDATE'?'修改提测':'新建提测'"/>
    </el-header>
    <el-main>
      <el-form :model="requestForm" :rules="requestRules" ref="ruleForm" label-width="100px" >
        ...
        <el-form-item label="提测版本" prop="test_version">
          <el-input v-model="requestForm.test_version" placeholder="部署版本号/分支/Tag" style="width: 350px"></el-input>
        </el-form-item>
  ...
  methods: {
    ...
    appSelected() {
      ...
      for (var it in this.appIdList) {
        if (this.appIdList[it].id === this.requestForm.appId) {
          ...
          if (this.requestForm.developer !== this.appIdList[it].developer) {
            this.requestForm.developer = this.appIdList[it].developer
          }
          if (this.requestForm.tester !== this.appIdList[it].tester) {
            this.requestForm.tester = this.appIdList[it].tester
          }
          if (this.requestForm.CcMail !== this.appIdList[it].CcEmail) {
            this.requestForm.CcMail = this.appIdList[it].CcEmail
          }
          if (this.requestForm.wiki !== this.appIdList[it].wiki) {
            this.requestForm.wiki = this.appIdList[it].wiki
          }
          if (this.requestForm.gitCode !== this.appIdList[it].gitCode) {
      ...
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          if (this.testAction === 'ADD') {
            ...
            console.log('commit' + this.version)
            this.requestForm.version = this.version
            this.requestForm.updateUser = this.op_user
    ...
    getTestInfo() {
      apiTestInfo(this.testId).then(response => {
        ...
        this.requestForm.test_version = data.test_version
