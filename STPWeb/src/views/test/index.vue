      ...
      <el-table :data="testData" v-loading="loading">
        ...
        <el-table-column label="操作" width="400">
          <template slot-scope="scope">
            ...
            <el-divider direction="vertical" />
            <el-link type="primary" @click="deleteTest(scope.row)">删除</el-link>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  methods: {
    ...
    doUpdate(row) {
      console.log('index' + row.version)
      this.$router.push({ path: '/commit?action=UPDATE&id=' + row.id + '&version=' + row.version })
    },
    startTest(row) {
      const reqBody = {
        id: row.id,
